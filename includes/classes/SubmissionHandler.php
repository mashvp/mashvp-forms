...
{
  public function registerFormHandler(): void
  {
  ...
  private function redirectToReferer(bool $success, string $message = '', $localizedMessage = ''): void
  {
    $args = [
    ...
    if ($localizedMessage) {
      $args['mvpf-message'] = rawurlencode((string) $localizedMessage);
    }
  ...
  private function generateJSONResponse(bool $success, ?string $message = '', $localizedMessage = ''): void
  {
    if (!$localizedMessage) {
      $localizedMessage = _x(
  ...
  private function terminateSubmissionProcess(
    bool $success,
    string $message = '',
    $localizedMessage = '',
  ): void {
  ...
  private function verifyRecaptchaToken($token)
  {
    ...
    if (curl_errno($ch) !== 0) {
      ...
      error_log(
        sprintf(
          '[mashvp-forms] Error verifying reCAPTCHA token: %s (from: %s)',
          $err,
          $remote_ip
        )
      );
    ...
    if (!$json) {
      error_log(
        sprintf(
          '[mashvp-forms] reCAPTCHA error: Unable to decode JSON response (from: %s)',
          $remote_ip
        )
      );
    ...
    if (!$json['success'] && is_array($json['error-codes'])) {
      ...
      error_log(
        sprintf(
          '[mashvp-forms] reCAPTCHA error: %s (from: %s)',
          $err,
          $remote_ip
        )
      );
  ...
  private function isRecaptchaSpam()
  {
    $remote_addr = new RemoteAddress();
    $remote_ip = $remote_addr->getIpAddress();
    ...
    if (!$token) {
      error_log(
        sprintf(
          '[mashvp-forms] reCAPTCHA error: No token was submitted but validation is enabled (from: %s)',
          $remote_ip
        )
      );
  ...
  private function isSpam(Form $form): string|false
  {
  ...
  private function logSpamAttempt(Form $form, string $reason = 'unknown'): void
  {
    $remote_addr = new RemoteAddress();
    ...
    error_log(
      sprintf(
        '[mashvp-forms] Spam attempt detected on form "%s" (reason: %s, from: %s)',
        $form_name,
        $reason,
        $remote_ip
      )
